    ...
    <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end gap-3">
      ...
      <div
        className={`flex flex-col bg-white border border-stone-200 rounded-3xl shadow-xl overflow-hidden transition-all duration-300 origin-bottom-right ${
          open
            ? "opacity-100 scale-100 w-80 sm:w-96 h-[500px]"
            : "opacity-0 scale-90 w-80 sm:w-96 h-[500px] pointer-events-none"
        }`}
      >
        {/* Header */}
      ...
      <button
        onClick={() => setOpen((o) => !o)}
        className="w-14 h-14 rounded-full bg-green-700 hover:bg-green-800 shadow-lg flex items-center justify-center transition btn-glow"
      >
        {open ? (
